...
class SevenTwelve extends React.Component{
...
submitForm(){
  fetch('http://127.0.0.1:8000/api/bseventwelve/',{
      method:'POST',
...
render(){
  ...
  <h1 className="text-3xl font-semibold text-center text-slate-800 underline uppercase decoration-bold mb-5">
               Part B Criteria 7 Table 7.3.1
            </h1>
    <h3 className="text-3xl mb-10 bg-gradient-to-r from-slate-100 to-slate-300 rounded-md shadow-xl  shadow-slate-900 ring-2 ring-slate-600">Portray the performance of the Institution in one area distinctive to its priority and thrust within 1000 words.</h3>
    ...
    <textarea className="textarea textarea-bordered w-full" placeholder="Write description here" value={this.state.description} name="description" onChange={this.changeHandler}></textarea>
        <table>
            <thead>
                <tr>
                    <th>
                    <textarea className="textarea textarea-bordered w-full" placeholder="Appropriate webpage in the Institutional website" value={this.state.webpage_link} name="webpage_link" onChange={this.changeHandler}></textarea>
                    </th>
                    <th>
                    <input type="text" placeholder="Any other relevant information" className="input-lg" value={this.state.add_info} name="add_info" onChange={this.changeHandler}/>
                    </th>
                </tr>
            </thead>
        </table>
