...
class SevenSix extends React.Component{
...
submitForm(){
  fetch('http://127.0.0.1:8000/api/bsevensix/',{
      method:'POST',
...
render(){
  ...
  <h1 className="text-3xl font-semibold text-center text-slate-800 underline uppercase decoration-bold mb-5">
               Part B Criteria 7 Table 7.1.6
            </h1>
    <h3 className="text-3xl mb-10 bg-gradient-to-r from-slate-100 to-slate-300 rounded-md shadow-xl  shadow-slate-900 ring-2 ring-slate-600">Quality audits on environment and energy are regularly undertaken
by the institution.</h3>
...
<select className="select-lg select-bordered w-full " value={this.state.initiative} name="initiative" onChange={this.changeHandler}>
<option disabled selected>Quality audits on environment.</option>
<option>Green audit /environment audit</option>
<option>Energy audit</option>
<option>Clean and green campus recognitions/awards </option>
<option>Beyond the campus environmental promotional activities </option>
</select>
</th>
<th scope="col" classNmae="px-6 py-4">
