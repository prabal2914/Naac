...
class SixTwo extends React.Component{
...
submitForm(){
  fetch('http://127.0.0.1:8000/api/bsixtwo/',{
      method:'POST',
...
render(){
  ...
  <h1 className="text-3xl font-semibold text-center text-slate-800 underline uppercase decoration-bold mb-5">
               Part B Criteria 6 Table 6.2.1
            </h1>
    <h3 className="text-3xl mb-10 bg-gradient-to-r from-slate-100 to-slate-300 rounded-md shadow-xl  shadow-slate-900 ring-2 ring-slate-600">The institutional perspective plan is effectively deployed and
functioning of the institutional bodies are effective and efficient as
visible from policies, administrative setup, appointment, service
rules, and procedures, etc.</h3>
    ...
    <textarea className="textarea textarea-bordered w-full" placeholder="Write description here" value={this.state.description} name="description" onChange={this.changeHandler}></textarea>
        <table>
            <thead>
                <tr>
                <th>
                    <textarea className="textarea textarea-bordered w-full" placeholder="Institutional perspective Plan" value={this.state.inst_docs} name="inst_docs" onChange={this.changeHandler}></textarea>
                    </th>
                    <th>
