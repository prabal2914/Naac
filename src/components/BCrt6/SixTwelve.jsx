...
class SixTwelve extends React.Component{
...
submitForm(){
  fetch('http://127.0.0.1:8000/api/bsixtwelve/',{
      method:'POST',
...
render(){
  ...
  <h1 className="text-3xl font-semibold text-center text-slate-800 underline uppercase decoration-bold mb-5">
               Part B Criteria 6 Table 6.5.3
            </h1>
    <h3 className="text-3xl mb-10 bg-gradient-to-r from-slate-100 to-slate-300 rounded-md shadow-xl  shadow-slate-900 ring-2 ring-slate-600">Institution  has adopted the following for  Quality assurance:</h3>
    <div classNmae="overflow-hidden">
      <table>
            <thead>
                <tr>
<th scope="col" classNmae="px-6 py-4">
<select className="select-lg select-bordered w-full " value={this.state.qual_insurance} name="qual_insurance" onChange={this.changeHandler}>
<option disabled selected>Quality Assurance</option>
<option>Academic and Administrative Audit (AAA) and follow up action taken</option>
<option>Conferences, Seminars, Workshops on quality conducted</option>
<option> Collaborative quality initiatives with other institution(s)</option>
<option>Orientation programme on quality issues for teachers and students</option>
<option>Participation in NIRF and other recognized ranking</option>
<option>Any other quality audit</option>
</select>
</th>
<th scope="col" classNmae="px-6 py-4">
